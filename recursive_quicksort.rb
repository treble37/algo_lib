...
def quicksort(a,n,comparison_count)
  #puts "n: "+n.to_s+" a_lng "+a.length.to_s
  # puts a.to_s
  if n==1 
    return a, comparison_count
  end
  p=choosepivot(a,0) #choose pivot as first element
  
  #if pivot not first element, swap as preprocessing step
  
  ##CASE 1:  Use pivot as last element
  # p=choosepivot(a,a.length-1) #choose pivot as last element
  # swap_array_elements(a,0,a.length-1)
  ##CASE 2:  Use median of 3 pivot rule
  a_first = a.first
 ...
 #   puts "right "+new_a[pivot_pos+1..new_a.length-1].to_s+" incount "+(new_a.length-(pivot_pos+1)).to_s+" rcount "+rightcount.to_s
    b=nil
    c,rightcount=quicksort(new_a[pivot_pos+1..new_a.length-1],new_a[pivot_pos+1..new_a.length-1].length,new_a.length-(pivot_pos+1))
    #puts " rcount "+rightcount.to_s
  when a.length-1
  #  puts "left "+new_a[0..pivot_pos-1].to_s+" incount "+pivot_pos.to_s+" lcount "+leftcount.to_s
    b,leftcount=quicksort(new_a[0..pivot_pos-1],new_a[0..pivot_pos-1].length,pivot_pos) #recursively sort 1st part
    c=nil
   ...
   # puts "left+right "+new_a[0..pivot_pos-1].to_s+"+"+new_a[pivot_pos+1..new_a.length-1].to_s
    ...
    b,leftcount=quicksort(new_a[0..pivot_pos-1],new_a[0..pivot_pos-1].length,count1) #recursively sort 1st part
    c,rightcount=quicksort(new_a[pivot_pos+1..new_a.length-1],new_a[pivot_pos+1..new_a.length-1].length,count2) #recursively sort 2nd part
